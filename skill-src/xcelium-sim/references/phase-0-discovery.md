...
1. 분석서를 처음 작성할 때, 그 시점 실행의 `tb_source`/`tb_provenance`를 분석서 헤더에 함께 기록해둔다. `sim_batch_run`/`sim_regression`/`sim_bridge_run`이 반환값에 이미 포함하고 있으므로 별도 조회 불필요 — 형태는 `{"files": [{"path", "sha256"}, ...], "combined_sha256"}`(테스트 자신의 파일 + 그 파일이 `` `include``/`import`하는 직접 의존 파일까지 커버, 2026-07-06 갱신 — 테스트 파일 자체는 안 바뀌고 그 파일이 쓰는 공유 시퀀스/인터페이스 파일만 바뀌어도 감지되어야 하므로).
2. 다음 디버깅에서 캐시된 분석서를 재사용하기 전, 이번 실행의 `tb_source.combined_sha256`과 분석서 헤더에 기록된 값을 비교한다. 어떤 특정 파일이 바뀌었는지 알고 싶으면 `files[].sha256`을 개별 비교한다.
3. 일치하면 TB 소스(및 그 의존 파일들)가 그대로라는 뜻이므로 캐시를 그대로 재사용. 다르면(또는 분석서에 애초에 기록이 없으면 — 이 스킴 이전에 작성된 분석서 등) TB 소스가 바뀌었을 수 있으므로 재작성한다.
