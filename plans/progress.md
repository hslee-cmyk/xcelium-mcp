...
## 2026-07-08 - skill-src/xcelium-sim tool 개수 드리프트 수정 (24→25)

### 배경

`xcelium-mcp-debug-workflow-v2.plan.md`를 업데이트하며 실제 소스 기준 tool 개수가 25개(24→25, F-3 `list_active_sessions` 추가로 인한 변화)임을 확인해놓고도, 사용자가 "skill은 업데이트할 필요없나?" 물었을 때 이 gap을 짚지 못함 — F-185(PNOOBJ 수정)는 skill 업데이트 불필요가 맞았지만, **그와 별개로 tool-map.md/SKILL.md 자체가 이미 stale(24개, list_active_sessions 미기재)했다는 사실**을 놓쳤다. 사용자가 직접 지적.

### 수정

- `skill-src/xcelium-sim/references/tool-map.md`: 제목/본문 24→25, "Sim Lifecycle (10)"→"(11)", `list_active_sessions` 신규 행 추가(F-3, 2026-07-07), 소스 감사 각주에 "고정값 아님, 재감사 필요" 문구 추가
- `skill-src/xcelium-sim/SKILL.md`: description/본문/tool-map 참조 행 3곳 24→25
- `cp -r skill-src/xcelium-sim ~/.claude/skills/`로 재배포, diff 확인

**Learnings:**
- tool 개수 같은 "감사 가능한 숫자"는 한 문서(plan)에서 고치면서 다른 문서(skill)의 동일 정보를 함께 확인하는 습관이 필요 — 같은 세션에서 발견한 사실이라도 다른 산출물에 자동 전파되지 않는다.
- 사용자가 "왜 이게 빠졌지?"라고 물었을 때 방어하지 않고 바로 인정 + 원인(gap을 연결짓지 못함) 설명 후 수정하는 것이 맞는 대응이었음.

---

## 2026-07-08 - list_active_sessions 용도 정정 + xcelium-mcp-debug-workflow-v2.plan.md v1.11 (CLI 설계 논리 충돌 발견)

### list_active_sessions 용도 정정

사용자가 "재기동과 관련된 게 아니라 SSH 끊김 시 재연결 대상 찾는 용도 아니냐"고 질문 → `xcelium-mcp-sim-session-reaper.design.md` §4.3(원본 설계)과 `xcelium-mcp-session-state-reattach.design.md`(재연결 메커니즘) 직접 대조. 결과: **둘 다 틀렸음** — 실제 재연결은 `connect_simulator(sim_dir=X)`가 registry에서 직접 포트 조회로 처리하고 `list_active_sessions`를 전혀 거치지 않는다. 이 tool의 실제 원 설계 목적은 "TTL 기반 자동 리퍼가 강제 종료하기 전에 사람이 방치된 세션을 보고 cleanup 여부를 판단"하는 것뿐. `server-ops.md`에 잘못 추가했던 §1.5(재기동 맥락 프레이밍)를 전체 제거하고, `tool-map.md` 설명만 설계 문서 원본 근거로 교정.

**Learnings:** 관련 있어 보이는 두 가정(내 프레이밍 vs 사용자 프레이밍) 모두 실제 설계 문서 대조 없이는 확인 불가 — 두 후보 모두 그럴듯해 보여도 원본 설계 문서(design.md)를 직접 읽어야 진짜 답이 나온다.

### xcelium-mcp-debug-workflow-v2.plan.md v1.11 — CLI(FR-06) 설계 논리 충돌

사용자가 "plan 문서에 논리적으로 업데이트 필요한 부분이 있나" 질문 → 소스 재검증 결과 진짜 설계 충돌 발견:

- v1.0~v1.10은 CLI를 `server.py:main()` 안에 `sys.argv` 분기(`len(sys.argv) > 1`)로 추가하는 안을 전제
- 하지만 이 plan 이후 실제 완료된 `xcelium-mcp-server-process-lifecycle` feature가 다른 컨벤션을 이미 확립: `pyproject.toml [project.scripts]`에 `xcelium-mcp-supervisor`/`xcelium-mcp-culler` 등 **독립 console_script 3개** 기존재, `stdio_forward.py`/`sim_session_reaper.py`도 독립 `-m` 모듈
- 결정적으로, supervisor는 연결마다 `_xcelium_server.main()`을 fork 후 **직접 함수 호출**(subprocess 재실행 아님)하므로 연결별 sys.argv 전달 경로 자체가 없음 — 원래 설계의 전제(`len(sys.argv) > 1`로 CLI/MCP 분기)가 이 배포 모델에서는 성립할 수 없음(다만 서로 독립된 프로세스라 실제 충돌은 없었음 — risk framing이 이제 무의미해진 것)

**수정**: FR-06/§3.4/§7(전면 재작성)/§8.1/§8.2 B단계/Impact Analysis(3개 표)/§11 리스크 테이블 갱신 — CLI를 `xcelium-mcp-cli` 독립 console_script로 재설계, `server.py`는 compound tool 3개 register()만 추가하도록 스코프 축소. "sys.argv 분기로 MCP 깨짐" 리스크 제거. 부수적으로 xcelium-mcp 자신의 루트 CLAUDE.md "Deployment" 섹션이 구 배포 모델을 그대로 보여주는 별개 문서 부채임도 확인(이 plan 범위 밖이라 Impact Analysis만 그 부채에 의존하지 않도록 일반화).

**Files changed:**
- docs/01-plan/features/xcelium-mcp-debug-workflow-v2.plan.md (v1.11)
- skill-src/xcelium-sim/references/server-ops.md (§1.5 제거)
- skill-src/xcelium-sim/references/tool-map.md (list_active_sessions 설명 정정)
- .bkit/state/pdca-status.json
- plans/progress.md (this entry)

**Learnings:**
- "논리적으로 업데이트 필요한 부분"을 물었을 때, 단순 재검증(파일 존재 확인)을 넘어 **다른 feature의 완료가 이 문서의 설계 전제 자체를 무효화했는지**까지 확인해야 진짜 가치 있는 발견이 나온다 — pyproject.toml의 기존 console_script 패턴이 결정적 근거였음.
- 두 skill 문서 수정(list_active_sessions) 모두 "이럴 것이다"라는 추측 대신 원본 design.md를 직접 대조한 뒤에야 정확한 결론이 나왔다 — 이 세션 전체에서 반복된 교훈.

---

## 2026-07-08 - xcelium-mcp-debug-workflow-v2.plan.md v1.12 (§5.1/§5.2 중복 발견)

사용자가 "다른 논리적 업데이트 필요한 부분 더 있나" 재질문 → 추가 소스 재검증에서 2건 발견:

1. **§5.1 sim-state.json 부분 중복**: `registry.py`에 이미 `update_session_state`/`get_session_state`(F-D, session-state-reattach)로 sim_dir별 `current_test_name`/`current_tb_source` 영속 추적이 구현돼 있음. v2 plan의 신규 `{project}/.ai/sim-state.json` 제안은 이 그릇의 존재를 모르고 쓰여짐 — Phase 2 Design 시 "새 파일 vs registry.py 확장" 결정이 선행돼야 한다는 각주 추가.
2. **§5.2 TB YAML frontmatter의 `last_verified`(수동 날짜) 필드**: F-175가 이미 `tb_source.combined_sha256` 자동 체크섬으로 이 문제를 더 잘 해결해놨음(`phase-0-discovery.md` §0C). 수동 필드를 자동 방식으로 교체.
3. (약한 발견) §6 Hook 언어 — JS 제안이 이 사용자의 실제 Python hook 컨벤션과 다름, Phase D 착수 시 재검토 각주만 추가.

**Files changed:**
- docs/01-plan/features/xcelium-mcp-debug-workflow-v2.plan.md (v1.12)
- .bkit/state/pdca-status.json
- plans/progress.md (this entry)

**Learnings:**
- "더 있나" 재질문에 답하려면 이미 훑은 섹션(§1~4, §7~11)이 아니라 **아직 안 본 섹션(§5, §6)**을 마저 검증해야 새 발견이 나온다 — 이전 라운드에서 다룬 부분을 재탕하지 않고 커버리지를 넓히는 방향으로 접근한 것이 유효했다.
- 두 발견 모두 같은 패턴: "이 plan이 제안하는 새 메커니즘이, 이 plan 작성 이후 다른 feature가 이미 구현한 더 나은 방법과 겹친다" — plan 문서 하나만 볼 게 아니라 그 사이 완료된 다른 feature들의 산출물과 항상 교차 검증해야 한다.

---

## 2026-07-08 - xcelium-mcp-debug-workflow-v2.plan.md v1.13 (문서 전체 커버리지 완료)

사용자가 "마저 해"로 재요청 → 남은 미검증 섹션(§10 향후 확장, Migration Note)을 마저 확인. 둘 다 forward-looking/순수 사실 기록이라 실질적 수정 없음(clean). 대신 v1.12에서 §5.1을 바꾼 여파로 FR-05와 Success Criteria DoD가 여전히 "sim-state.json"을 확정 파일명처럼 서술하던 하위 불일치를 발견해 "신규 파일 vs registry.py 확장, §5.1 참조"로 정합화. 본문 곳곳(§2/§4/§5/§8 등)의 "sim-state.json" 개념적 언급 자체는 그대로 뒀다 — 실제 저장 메커니즘 결정은 Phase 2 Design에 위임된 구현 세부사항이라, 지금 전부 재작성하는 건 과도한 선제 작업으로 판단.

**Files changed:**
- docs/01-plan/features/xcelium-mcp-debug-workflow-v2.plan.md (v1.13)
- .bkit/state/pdca-status.json
- plans/progress.md (this entry)

**Learnings:**
- 문서 전체를 훑는 재검증 작업에서, 한 섹션(§5.1)을 고치면 그 개념을 참조하는 다른 섹션(FR-05, DoD)까지 하위 정합성이 깨질 수 있다는 걸 놓치지 않고 확인 — 매 라운드 "이번에 바꾼 게 다른 곳에도 영향을 주는가"를 체크하는 습관이 유효했다.
- 모든 하위 참조를 다 rewrite하지 않고, 결정이 아직 안 난 것(저장 메커니즘)은 "개념적 이름은 유지 + 결정 지점(§5.1)만 명시적으로 가리키기"로 처리 — 미확정 사항을 문서 전체에 억지로 확정된 것처럼 퍼뜨리지 않는 절제가 더 나은 결과.
- 이것으로 이 plan 문서의 전 섹션(§1~11 + Requirements/Dependencies/Impact Analysis/Success Criteria/Migration Note) 재검증 라운드가 완료됨 — v1.10~v1.13, 총 4회 개정.

---

## 2026-07-08 - xcelium-mcp-debug-workflow-v2.plan.md v1.14 (§6.4 중복 다이어그램 누락분 수정)

사용자가 "hook 파일구조의 diagram이 stale함"이라고 지적 → §6.4 "Hook 파일 구조"가 §4.3과 똑같은 내용을 그리는 **중복 다이어그램**인데, v1.10에서 §4.3만 실제 Phase 1 구조로 갱신하고 §6.4는 그대로 뒀던 것을 발견(여전히 run-guide.md/analyze-guide.md/debug-guide.md 표시). §4.3과 동일한 실제 구조(phase-0~5+tool-map+server-ops+backend-interface)로 교체. 이어서 grep으로 나머지 문서 전체를 재검색해 §8.2 Phase C 구현 순서(C-2~C-4가 존재하지 않는 파일 작성을 지시)도 같은 원인의 잔재임을 발견해 함께 정합화 — backend-interface.md 작성 + TB frontmatter(§5.2 새 형식) 갱신만 남기고 나머지 단계 제거.

**Files changed:**
- docs/01-plan/features/xcelium-mcp-debug-workflow-v2.plan.md (v1.14)
- .bkit/state/pdca-status.json
- plans/progress.md (this entry)

**Learnings:**
- **같은 정보를 그리는 다이어그램/목록이 문서 내 여러 곳에 있으면, 하나를 고칠 때 나머지를 grep으로 찾아 다 같이 고쳐야 한다** — v1.10에서 §4.3만 고치고 §6.4를 놓친 게 이번에 재발한 사례. 앞으로 파일 구조를 바꿀 땐 `grep -n "<옛 파일명>"`으로 전체 문서를 훑는 걸 마지막 단계로 습관화.
- 사용자가 구체적으로 "이 부분이 stale하다"고 짚어줄 때는, 그 지적이 가리키는 정확한 섹션뿐 아니라 **같은 원인으로 stale해진 다른 곳**도 함께 찾아야 재작업을 피할 수 있다(§8.2 Phase C가 그 사례).

---

## 2026-07-22 - F-188: hooks(PostToolUse/UserPromptSubmit) 등록 커맨드의 python3 하드코딩으로 Windows에서 hook 전혀 미발동

### 배경
xcelium-mcp-debug-workflow-v2 Plan §8.2 E-6(Hook 실제 발동 확인) 수동 검증 중 발견. `skill-src/xcelium-sim/SKILL.md`의 `hooks:` frontmatter가 `python3 ~/.claude/skills/xcelium-sim/hooks/....py`로 등록돼 있는데, 이 Windows 개발 머신에서 `python3`는 Microsoft Store App Execution Alias stub으로 연결되어 있어 실제로 실행하면 "Python was not found; run without arguments to install from the Microsoft Store..." 메시지와 함께 비정상 종료(rc=49)한다. PostToolUse/UserPromptSubmit은 실패를 조용히 삼키는 fail-open 설계라 사용자에게 에러가 안 보였을 뿐, 이 hook은 이 환경에서 한 번도 실행된 적이 없었다.

### 구현
- `SKILL.md`의 두 `command:` 값을 `python3 <script> || python <script>` fallback 패턴으로 변경 — `python3`가 실패(비정상 종료)하면 `python`으로 재시도. 이 머신에서는 `python`이 `C:/Python314/python.exe`를 정상적으로 가리키므로 fallback만으로 즉시 해결됨.
- SKILL.md 본문에 Windows 캐비어 안내 콜아웃 추가 — `python`도 없거나 깨져 있으면 `command:` 값을 자신의 python 실행파일 절대경로로 직접 바꿔야 한다는 안내(hook은 fail-open이라 실패해도 조용히 무시되므로, 증상은 "다음 단계 제안"이 그냥 안 뜨는 것으로만 나타난다는 점도 명시).
- `hooks/test_hooks.py`에 `TestRegisteredHookCommand` 클래스 신규 — 기존 테스트들은 `sim_post_compound.main()`/`sim_prompt_detect.main()`을 in-process로 직접 호출해서 등록된 **command 문자열 자체**는 한 번도 검증하지 않았다는 gap을 발견하고, SKILL.md frontmatter에서 두 command를 정규식으로 추출해 (1) 둘 다 등록돼 있는지 (2) 각각 `python3`→`python` fallback을 갖고 있는지 정적으로 검증하는 회귀 테스트 2개 추가.

### 결과
- `hooks/test_hooks.py`: 15 passed(13 기존 + 2 신규), 회귀 없음
- `pytest tests/`: 712 passed, 회귀 없음
- `ruff check src/`: clean (`skill-src/xcelium-sim/hooks/`도 별도로 clean 확인)
- 배포(`~/.claude/skills/xcelium-sim/`로 cp -r 재배포)는 F-189(같은 hooks 파일들의 stdin 인코딩 버그) 수정과 묶어서 한 번에 진행 예정 — 같은 파일을 두 번 재배포하지 않기 위함.

### Files changed
- skill-src/xcelium-sim/SKILL.md (hooks: command fallback + Windows 안내 콜아웃)
- skill-src/xcelium-sim/hooks/test_hooks.py (TestRegisteredHookCommand 신규)
- plans/progress.md (this entry)

**Note (prd.json `passes` 필드 미변경)**: 사용자 memory 규칙(F-185 세션에서도 동일하게 적용됨)에 따라 `passes: true`/`completed_at`은 사용자가 직접 검토 후 갱신 — 이번 커밋에서 건드리지 않음.

**Learnings:**
- 스킬의 `hooks:` frontmatter처럼 **Claude Code 런타임이 자동으로 실행하는 커맨드 문자열**은, 그 문자열이 호출하는 스크립트의 로직을 in-process로 테스트하는 것만으로는 검증되지 않는다 — 실제 shell에서 그 문자열 자체가 해석 가능한지는 완전히 별개의 검증 대상이다. 이번 F-188도, F-189(다음 항목)도 둘 다 "스크립트 로직은 정상, 하지만 실제 실행 경로(interpreter 해석/stdin 인코딩)가 깨져 있어 도달조차 못 함"이라는 같은 종류의 gap.
- PostToolUse/UserPromptSubmit 같은 fail-open hook은 설계상 안전하지만, 그 대가로 **고장이 완전히 무음(silent)** 이라는 트레이드오프가 있다 — 이번 세션 내내 hook의 "다음 단계 제안"이 한 번도 안 뜨는 걸 그냥 지나칠 뻔했고, 명시적으로 E-6 항목을 검증하려고 직접 command를 재현해보고 나서야 발견됐다. 향후 유사한 fail-open 메커니즘을 만들 때는 이번처럼 "등록된 문자열 자체"를 검증하는 정적 테스트를 처음부터 같이 추가하는 습관이 필요하다.

---

## 2026-07-22 - F-189: sim_prompt_detect.py stdin이 cp949로 잘못 디코딩되어 한글 트리거 키워드 전부 매치 실패

### 배경
F-188과 같은 E-6 검증 세션에서 연이어 발견. `sys.stdin`의 기본 인코딩은 플랫폼 로케일을 따르는데(UTF-8이 아님), 이 Windows 개발 머신에서는 `cp949`(한국어 코드페이지)였다. `json.load(sys.stdin)`은 이 불일치에서 예외를 던지지 않는다 — surrogateescape로 조용히 mojibake 문자열을 만들어 "성공"한다. 그 결과 `_TRIGGER_KEYWORDS`의 한글 항목(시뮬레이션/디버깅/재기동 등)이 이 환경에서 전부 매치 실패했다(영문 키워드만 정상 동작).

### 구현
- `sim_prompt_detect.py`/`sim_post_compound.py` 둘 다에 `_read_stdin_json()` 헬퍼 추가 -- `sys.stdin.buffer`(원시 바이트 스트림)를 직접 읽어 UTF-8로 명시적으로 디코딩, 플랫폼 로케일이 개입할 여지를 없앤다. `.buffer`가 없는 경우(테스트의 `io.StringIO` monkeypatch)는 `sys.stdin.read()`로 폴백해 기존 테스트를 깨지 않음.
- 두 파일 다 별도 공용 모듈로 추출하지 않고 각자 자체 포함(self-contained) 스크립트 철학을 유지하며 각각에 동일한 헬퍼를 둠(작은 중복, 스코프 최소화).
- `hooks/test_hooks.py`에 `TestStdinEncoding` 클래스 신규 -- `PYTHONIOENCODING=cp949`로 강제한 실제 subprocess를 스폰해(이 env var는 OS 로케일과 무관하게 모든 플랫폼에서 동일하게 동작하므로 완전히 이식 가능한 재현 방법) 실제 UTF-8 바이트로 인코딩된 한글 키워드를 stdin에 흘려보내고 정상 매치되는지 검증하는 테스트 3개 추가: (1) 한글 키워드가 강제된 cp949 환경에서도 매치되는지, (2) 기존 영문 키워드가 회귀 없이 계속 매치되는지, (3) `sim_post_compound.py`도 동일 환경에서 정상 동작하는지.
- **Sanity check(수정이 실제로 뭔가를 고쳤는지 검증)**: `git stash`로 수정 전 코드를 잠깐 복원해 신규 테스트 3개를 재실행 — 한글 키워드 테스트가 정확히 기대한 대로 FAIL(반환값 없음)하는 것을 확인한 후 `git stash pop`으로 수정 복원. 나머지 2개(영문 키워드, post_compound)는 애초에 ASCII-only라 old 코드에서도 우연히 통과 — 이 사실도 확인해 테스트가 정확히 어떤 조건에서만 버그를 드러내는지 명확히 함.

### 결과
- `hooks/test_hooks.py`: 18 passed(15 기존+신규 F-188 테스트 + 3 신규 F-189 테스트), 회귀 없음
- `pytest`(전체 스위트): 715 passed, 회귀 없음
- `ruff check src/` + `skill-src/xcelium-sim/hooks/`: clean
- 배포(`~/.claude/skills/xcelium-sim/`로 재배포)는 F-188과 F-189를 묶어서 한 번에 진행

### Files changed
- skill-src/xcelium-sim/hooks/sim_prompt_detect.py (`_read_stdin_json()` 추가)
- skill-src/xcelium-sim/hooks/sim_post_compound.py (동일 헬퍼 추가)
- skill-src/xcelium-sim/hooks/test_hooks.py (`TestStdinEncoding` 신규 3개)
- plans/progress.md (this entry)

**Note (prd.json `passes` 필드 미변경)**: F-188과 동일 -- 사용자가 직접 검토 후 갱신.
...
## 2026-07-22 - F-186 Rev.2: 첫 수정이 실제 발견 당시의 정확한 메시지를 놓침 (사용자가 재검증 요청해 발견)

### 배경
사용자가 "실제 이 문제를 발견했던 상황에서 다시 검증해"라고 요청 — F-186/187/188/189를 각각 실제 배포 서버(cloud0, push+pull+supervisor 재기동)와 실제 최초 발견 데이터로 재검증하는 과정에서, F-186의 첫 수정(`"failed!!" in log.lower()`)이 **정작 이 버그를 최초로 발견하게 만든 그 메시지 자체를 놓친다**는 것을 발견했다: TID_TOP010_register_bank_test의 bounded-timeout guard가 실제로 찍는 메시지는 `"[REG BANK TEST] register 0x05: back-tel NOT sent (timeout)!!"`인데, 이 문자열엔 "failed"가 전혀 없다 — 오직 "[TAG] ...!!" 컨벤션 자체만 verdict 표시다. 첫 수정본의 신규 테스트 4개는 전부 "failed!!"라는 문자열을 직접 포함한 예시로만 만들어져서, 이 gap을 안 잡고 그대로 통과·커밋·cloud0 배포까지 됐었다.

### 재검증 방법
1. F-187: 실제 배포 서버에 `sim_disconnect(action="bridge", target="all")`을 미연결 상태로 실제 호출 → `"No bridge connected."` 정상 확인.
2. F-186: cloud0에 남아있는 **원본 hang 로그**(`/tmp/xcelium_mcp_1001/regression_1784702932_TID_TOP010_register_bank_test.log`, 34MB)를 배포된 `_classify_status()`에 직접 통과시켜봄 — 이 로그 자체는 `$finish`가 아예 없어서(진짜 hang이라 SIGTERM으로 죽인 로그) 수정 전 코드에서도 우연히 FAIL이 나오는 로그라 판별력이 없었다. 그래서 TB 소스(`TID_TOP010_register_bank_test.v:101/133`)에서 **실제 타임아웃 가드 메시지의 정확한 문자열**을 그대로 가져와 `$finish`가 정상 도달한 상황을 재구성해 배포된 함수에 통과시켰더니 — **PASS로 오분류됨을 실측 확인**(회귀 재현).

### 구현 (Rev.2)
- 문자열 리터럴 매칭(`"failed!!" in log.lower()`)을 `_TB_BANG_LINE_RE = re.compile(r'^\[[^\]]+\].*!!', re.MULTILINE)`로 교체 — 이 TB 계열의 실제 컨벤션 자체(대괄호 `[TAG]`로 시작하고 `!!`를 포함하는 줄)를 일반화해서 잡는다. 매치된 줄들 중 "passed"가 없는 줄이 하나라도 있으면 FAIL. `[TAG]` 줄-시작 앵커 덕분에 로그 어딘가의 무관한 "!!" 노이즈에는 오탐하지 않는다(신규 테스트로 확인).
- `tests/test_compound.py`에 실제 발견 메시지를 그대로 쓴 재현 테스트(`test_real_timeout_guard_message_is_fail`)와 오탐 방지 테스트(`test_unrelated_bang_noise_without_bracket_tag_does_not_misfire`) 2개 추가(총 6개).

### 결과
- `tests/test_compound.py::TestClassifyStatus`: 6 passed(이전 4개 + 신규 2개)
- 전체 `pytest`: 723 passed, 회귀 없음
- `ruff check src/`: clean
- 재배포(push→cloud0 pull→supervisor 재기동) 후 실제 원본 발견 메시지로 재확인 예정(이 커밋 직후 진행)

### Files changed
- src/xcelium_mcp/compound.py (`_TB_BANG_LINE_RE`로 매칭 로직 일반화)
- tests/test_compound.py (신규 테스트 2개, 클래스 docstring에 Rev.2 경위 기록)
- plans/progress.md (this entry)

**Learnings:**
- **이 세션에서 가장 값진 교훈**: 유닛 테스트가 전부 통과하고 sanity-check(git stash)까지 했어도, 그 테스트들이 **실제 버그를 발견하게 만든 정확한 원본 데이터/메시지**로 짜여 있지 않으면 여전히 진짜 gap을 놓칠 수 있다 — 이번 경우 내가 만든 4개 테스트 전부 "failed!!"라는 문자열을 직접 예시에 박아 넣었는데, 정작 실제 버그 트리거는 다른 문구("NOT sent (timeout)!!")였다. **버그 리포트에 인용된 "재현 근거" 원문(로그 라인, 에러 메시지 등)을 재구성이나 요약 없이 테스트에 그대로 박아 넣는 습관**이 필요하다 — 합리적으로 보이는 유사 예시를 만드는 것만으로는 부족하다.
- 사용자가 "실제 발견 상황에서 재검증"을 요청한 게 정확히 이 gap을 잡아냈다 — 수정을 "구현했다"와 "실제로 원래 문제를 해결했다"는 서로 다른 주장이며, 후자는 원본 재현 조건으로 직접 확인해야만 검증된다.

---

## 2026-07-23 - F-191: classify_regression_results()가 legacy directed test의 '[TAG] ...!!' 컨벤션을 인식 못해 regression에서 'complete'로 오분류 (F-186과 동일 계열, 별도 함수)

### 배경
F-186/F-186 Rev.2는 `compound.py::_classify_status()`(단일 테스트 경로, `sim_run_and_check` 등이 사용)만 고쳤는데, `batch_runner.py::classify_regression_results()`(regression 경로, `sim_regression`이 사용)는 **완전히 별개의 함수**로 동일한 맹점을 독립적으로 갖고 있었다 — COMPLETE./대문자 FAIL 어느 것도 없고 `$finish`엔 도달했지만 legacy TB의 `[TAG] ...!!`(not passed) verdict 라인이 있는 경우, regression 집계에서 여전히 "complete"로 잘못 분류됨. `run_regression에서 각 테스트의 결과가 따로 오지 않는 문제`를 사용자가 물어 TODO.md에서 관련 기록(F-190 대상)을 찾던 중, 그와는 별개로 이 gap을 발견해 사용자 승인("응, 등록해줘")으로 F-191 등록.

### 구현
- F-186 Rev.2에서 쓰던 `_TB_BANG_LINE_RE = re.compile(r'^\[[^\]]+\].*!!', re.MULTILINE)`를 `compound.py`에서 `batch_runner.py`(`_COMPLETE_RE` 바로 아래)로 이동 — compound.py는 이미 `_COMPLETE_RE`를 batch_runner.py에서 import해 쓰고 있어 공유 모듈 소유권이 자연스러움. compound.py는 이제 자체 정의 없이 import만 해서 재사용(중복 정의 제거).
- `classify_regression_results()`의 `elif "$finish" in t_raw:` 분기에 동일 검사 추가 — `_TB_BANG_LINE_RE.findall(t_raw)` 중 "passed"가 없는 라인이 하나라도 있으면(`has_bang_failure`) `complete_count` 대신 `error_count`로 집계.
- **F-186 Rev.2의 교훈을 그대로 적용**: 신규 테스트(`test_no_verdict_finish_with_real_tb_bang_line_is_error`)에 TID_TOP010_register_bank_test의 **실제 원본 메시지**("[REG BANK TEST] register 0x05: back-tel NOT sent (timeout)!!")를 그대로 사용 — 파라프레이즈나 합성 예시("failed!!") 금지.
- **Sanity check**: `git stash push -- src/xcelium_mcp/batch_runner.py`로 수정 전 코드를 복원한 뒤 신규 테스트를 재실행 — 정확히 예상대로 `AssertionError: assert '0/1 waveform tests COMPLETE' in '1/1 waveform tests COMPLETE\n\n...'`로 실패(수정 전엔 이 실제 시나리오가 "1/1 complete"로 오분류됨을 실측 확인)한 뒤 `git stash pop`으로 복원.

### 결과
- `tests/test_compound.py` + `tests/test_regression_classification.py`: 37 passed(신규 1개 포함)
- 전체 `pytest`: 724 passed, 회귀 없음
- `ruff check src/`: clean

### Files changed
- src/xcelium_mcp/batch_runner.py (`_TB_BANG_LINE_RE` 신설 + `classify_regression_results()` `$finish` 분기에 검사 추가)
- src/xcelium_mcp/compound.py (로컬 `_TB_BANG_LINE_RE` 정의 제거, batch_runner.py에서 import로 전환)
- tests/test_regression_classification.py (신규 테스트 1개, 실제 원본 메시지 사용)
- plans/progress.md (this entry)

**Note (prd.json `passes` 필드 미변경)**: 프로젝트 규칙대로 사용자가 직접 검토 후 갱신 — 단, F-186/187/188/189는 이미 이전 재검증 사이클에서 사용자가 명시적으로 `passes:true` 갱신을 지시한 바 있음.
