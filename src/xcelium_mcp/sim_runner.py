"""sim_runner.py — Core simulation lifecycle for xcelium-mcp v4.4.

v4.2: Functions split into env_detection.py, registry.py, batch_runner.py.
v4.4: Core shell utilities extracted to shell_utils.py.
      Tcl preprocessing extracted to tcl_preprocessing.py.

This file retains: start_bridge_simulation, _start_bridge,
run_full_discovery orchestrator, legacy script patching, and utility functions.

Re-exports from shell_utils and new modules are provided for backward
compatibility with tools/*.py imports that reference sim_runner.
"""
